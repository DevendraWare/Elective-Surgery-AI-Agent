...
def init_db():
    """Initialize the database with the required schema, including conversation history."""
    conn = get_db()
    try:
        c = conn.cursor()
        c.execute("""
...
def save_context_to_db():
    ...
    try:
        c = conn.cursor()
        context_json = json.dumps(st.session_state.conversation_context)
        session_id = "default_session"  # You can later replace with dynamic user session ID
        c.execute("""
            INSERT INTO conversation_history (session_id, context)
            VALUES (?, ?)
...
def load_context_from_db():
    """Load conversation context from the database when the app starts."""
    conn = get_db()
    try:
        c = conn.cursor()
        c.execute("SELECT context FROM conversation_history WHERE session_id = ?", ("default_session",))
        row = c.fetchone()
        if row:
            st.session_state.conversation_context = json.loads(row[0])
    finally:
        conn.close()
...
class PatientAnalyzer:
    ...
    def analyze_patients(self, patient_data, query):
        ...
        try:
            ...
            # Add full chat history to maintain memory
            for msg in st.session_state.conversation_context:
                messages.append({"role": msg["role"], "content": msg["content"]})

            # Include new user query
...
if "messages" not in st.session_state:
    ...
    st.session_state.conversation_context = []
    load_context_from_db()  # 🔹 Load previous conversation memory
